...
def update(dt):
  ...
  voltagedata = [abs(i*VoltsPerBit-1.25) for i in displayintdata]
  
  #print mean of voltage data
  print(np.mean(voltagedata[-100::]))
  
  #if statement to determine whether voltage surpasses flexing threshold or not
  if np.mean(voltagedata[-100::]) > 0.045:
      flexing = True
  else:
      flexing = False
